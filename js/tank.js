...
import { BLOCK_SIZE } from './constants.js?v=23';
import { state, tanks, particles } from './state.js?v=23';
import { getSurfaceY } from './terrain.js?v=23';
import { playSound } from './audio.js?v=23';
import { spawnHitParticles, spawnExplosion, Particle } from './particles.js?v=23';
import { updateUI, checkVictory, showAnnouncement, showDamagePopup } from './ui.js?v=23';
...
export class Tank {
    ...
    buildVoxelMesh() {
        ...
        const chassis = new THREE.Mesh(chassisGeom, this.chassisMat);
        ...
        const turret = new THREE.Mesh(turretGeom, this.chassisMat);
        ...
        this.barrelPivot.position.set(0, 0.22, 0);
    ...
    updateMeshPosition() {
        ...
        // Idle bob when selected
        const bobOffset = (state.selectedTank === this)
            ? Math.sin(performance.now() * 0.0022) * 0.09
            : 0;

        this.mesh.position.set(targetX, targetY + bobOffset, targetZ);
        ...
        // Apply barrel recoil offset
        if (this.barrelMesh) {
            this.barrelMesh.position.z = 0.6 + this.barrelRecoilOffset;
        }
    ...
    update(dt) {
        ...
        if (this.barrelRecoilOffset < 0) {
            this.barrelRecoilOffset = Math.min(0, this.barrelRecoilOffset + dt * 2.2);
            if (this.barrelMesh) this.barrelMesh.position.z = 0.6 + this.barrelRecoilOffset;
        }
        // HP bar always faces camera
        if (this.hpBarGroup) {
            this.hpBarGroup.quaternion.copy(state.camera.quaternion);
        }
    }

    triggerRecoil() {
        this.barrelRecoilOffset = -0.38;
        if (this.barrelMesh) this.barrelMesh.position.z = 0.6 + this.barrelRecoilOffset;
    }
    ...
    updateHealthBar() {
        ...
        this.hpBarFg.material.color.setHex(hpPercent < 0.35 ? 0xf43f5e : 0x10b981);
    }
    ...
    takeDamage(amount) {
        ...
        // Positional audio: pan based on X position on map
        const mapCenterX = 32;
        const pan = Math.max(-1, Math.min(1, (this.x * BLOCK_SIZE - mapCenterX) / 32));
        spawnHitParticles(this.x * BLOCK_SIZE, this.y * BLOCK_SIZE, this.z * BLOCK_SIZE, this.color);
        showDamagePopup(amount, this.mesh, this.color);

        // White hit flash
        if (this.chassisMat) {
            this.chassisMat.emissive.set(0xffffff);
            this.chassisMat.emissiveIntensity = 2.5;
            setTimeout(() => {
                if (this.chassisMat) {
                    this.chassisMat.emissive.set(0x000000);
                    this.chassisMat.emissiveIntensity = 0.0;
                }
            }, 200);
        }
    ...
    destroy() {
        ...
        // Voxel shatter: spawn body parts as debris particles
        const shardColors = [this.color, this.color, 0x1e293b, 0x0f172a, 0x334155, 0x94a3b8];
        for (let i = 0; i < 18; i++) {
            const size = 0.28 + Math.random() * 0.52;
            const life = 0.9 + Math.random() * 1.3;
            ...
            particles.push(new Particle(pos.x, pos.y + 0.5, pos.z, shardColor, size, velocity, life));
        }

        spawnExplosion(pos, this.color, 50);
        playSound('explosion');
        ...
        // Trigger victory cinematic if game just ended
        if (state.isGameOver && tanks.length > 0) {
            state.victoryCinematicTarget = tanks[0];
            state.victoryCinematicActive = true;
            state.victoryOrbitAngle = 0;
            setTimeout(() => playSound('victory_fanfare'), 600);
        }
